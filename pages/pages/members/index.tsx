...
const Index = () => {
    ...
    const exportPDF = () => {
        ...
        const headers = [['ID', 'LibraryID', 'Name', 'Type', 'Branch', 'Mobile', 'Status']];
        let data: any = products.map((obj: any) => {
            let utype = getType(obj.type);
            let ubranch = getBranch(obj.branch);
            let arrdata: any[][] = [obj.id, obj.libid, obj.name, utype, ubranch, obj.mobile, obj.status];
            return arrdata;
        });
    ...
    const rightToolbarTemplate = () => {
        return (
            <React.Fragment>
                ...
                <Button label="PDF" icon="pi pi-upload" className="inline-block mr-2" onClick={exportPDF} />
